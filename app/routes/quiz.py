...
async def student_pending_quizzes(request: Request, db=Depends(get_db)):
    """Get list of pending quizzes for the student (not yet attempted)."""
    user = await _require_student(request, db)
    student_id = user["id"]

    quizzes = await get_pending_quizzes_for_student(db, student_id)
...
async def student_get_quiz(quiz_id: int, request: Request, db=Depends(get_db)):
...
async def student_start_quiz(quiz_id: int, request: Request, db=Depends(get_db)):
    """
...
async def student_submit_quiz(quiz_id: int, body: QuizSubmission, request: Request, db=Depends(get_db)):
    """
...
async def student_get_quiz_results(quiz_id: int, request: Request, db=Depends(get_db)):
...
async def teacher_get_latest_attempt(
    ...
    # Get full summary
    summary = await get_attempt_summary(db, attempt["id"])
...
async def teacher_student_quiz_history(student_id: int, request: Request, db=Depends(get_db)):
