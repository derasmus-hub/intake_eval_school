...
async def generate_path(student_id: int, request: Request, db=Depends(get_db)):
...
async def get_learning_path(student_id: int, request: Request, db=Depends(get_db)):
...
async def update_week_progress(student_id: int, body: WeekUpdate, request: Request, db=Depends(get_db)):
