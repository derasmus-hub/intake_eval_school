...
async def get_due_cards(student_id: int, request: Request, db=Depends(get_db)):
...
async def add_card(student_id: int, card: VocabCard, request: Request, db=Depends(get_db)):
    user = await require_student_owner(request, student_id, db)
    # Check for duplicate
    cursor = await db.execute(
    ...
    await update_challenge_progress(db, student_id, "vocab_add")

    return {"id": cursor.lastrowid, "word": card.word, "status": "added"}


@router.post("/{student_id}/review")
async def submit_review(student_id: int, review: ReviewSubmission, request: Request, db=Depends(get_db)):
    user = await require_student_owner(request, student_id, db)
    cursor = await db.execute(
    ...
    await award_xp(db, student_id, 10, "vocab_review", f"Reviewed: {card['word']}")
    await update_challenge_progress(db, student_id, "review_vocab")
...
async def get_vocab_stats(student_id: int, request: Request, db=Depends(get_db)):
    user = await require_student_owner(request, student_id, db)
    cursor = await db.execute(
