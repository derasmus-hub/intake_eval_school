...
@router.post("/start")
async def start_assessment(body: StartAssessmentRequest, request: Request, db=Depends(get_db)):
    ...
    cursor = await db.execute(
        "SELECT id, name FROM users WHERE id = ?", (body.student_id,)
    )
...
async def submit_placement(submission: PlacementSubmission, request: Request, db=Depends(get_db)):
    ...
    if user["role"] == "student" and user["id"] != assessment["student_id"]:
        raise HTTPException(status_code=403, detail="Access denied")
...
async def submit_diagnostic(submission: DiagnosticSubmission, request: Request, db=Depends(get_db)):
    ...
    # Get student info
    cursor = await db.execute(
        "SELECT * FROM users WHERE id = ?", (submission.student_id,)
    )
    student = await cursor.fetchone()
    if not student:
        raise HTTPException(status_code=404, detail="Student not found")
    ...
    # Log to cefr_history for tracking level changes over time
    if determined_level:
        # Extract per-skill levels from sub_skill_breakdown
        skill_levels = {}
        for entry in sub_skill_breakdown:
            skill_name = entry.get("skill", "").lower()
            skill_level = entry.get("level")
        ...
        await db.execute(
            """INSERT INTO cefr_history
               (student_id, level, grammar_level, vocabulary_level,
                ...
                confidence, source, assessment_id)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'assessment', ?)""",
            (
                submission.student_id,
                determined_level,
                skill_levels.get("grammar"),
                skill_levels.get("vocabulary"),
                skill_levels.get("reading"),
                skill_levels.get("speaking"),
                skill_levels.get("writing"),
                ai_result.get("confidence_score"),
                submission.assessment_id,
            ),
        )
...
async def get_latest_assessment(student_id: int, request: Request, db=Depends(get_db)):
...
async def get_assessment(student_id: int, request: Request, db=Depends(get_db)):
