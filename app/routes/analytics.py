...
async def get_skill_analytics(student_id: int, request: Request, db=Depends(get_db)):
...
async def get_timeline(student_id: int, request: Request, db=Depends(get_db)):
    user = await require_student_owner(request, student_id, db)
    cursor = await db.execute(
...
async def get_achievements(student_id: int, request: Request, db=Depends(get_db)):
    user = await require_student_owner(request, student_id, db)
    # Check for new achievements using the comprehensive checker
    newly_earned = await check_achievements(db, student_id)
