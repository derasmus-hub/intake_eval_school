...
async def generate_word_match(student_id: int, request: Request, db=Depends(get_db)):
...
async def generate_sentence_builder(student_id: int, request: Request, db=Depends(get_db)):
...
async def generate_error_hunt(student_id: int, request: Request, db=Depends(get_db)):
...
async def generate_speed_translate(student_id: int, request: Request, db=Depends(get_db)):
...
async def submit_game_score(student_id: int, submission: GameSubmission, request: Request, db=Depends(get_db)):
    user = await require_student_owner(request, student_id, db)
    ...
    xp_result = await award_xp(db, student_id, xp, "game_complete", f"{submission.game_type}: {submission.score}%")
...
async def get_game_history(student_id: int, request: Request, db=Depends(get_db)):
    user = await require_student_owner(request, student_id, db)
    cursor = await db.execute(
