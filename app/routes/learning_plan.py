...
async def student_get_latest_plan(request: Request):
    """
...
async def student_get_plan_history(request: Request, limit: int = 10):
    """
    ...
    history = await get_plan_history(student_id, limit=limit)
...
async def student_get_specific_plan(plan_id: int, request: Request):
    """
...
async def teacher_get_student_plan(student_id: int, request: Request):
    """
    ...
    try:
        cursor = await db.execute(
            "SELECT id, name, current_level FROM students WHERE id = ? AND role = 'student'",
            (student_id,)
        )
...
async def teacher_refresh_student_plan(student_id: int, request: Request):
    """
    ...
    try:
        cursor = await db.execute(
            "SELECT id, name FROM students WHERE id = ? AND role = 'student'",
            (student_id,)
        )
        student = await cursor.fetchone()
        if not student:
