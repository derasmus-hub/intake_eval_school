...
async def submit_progress(lesson_id: int, entry: ProgressEntry, request: Request, db=Depends(get_db)):
    user = await get_current_user(request, db)

    # student_id is ALWAYS taken from JWT — never trust request body
    student_id = user["id"]
    ...
    await award_xp(db, student_id, xp_amount, "lesson_complete", f"Lesson {lesson_id}: {entry.score}%")

    # Update streak and check achievements
    await update_streak(db, student_id)
    await check_achievements(db, student_id, {"action": "lesson_complete", "score": entry.score})

    # Update challenge progress
    await update_challenge_progress(db, student_id, "complete_lesson")
    if entry.score and entry.score >= 90:
        await update_challenge_progress(db, student_id, "high_score")
...
async def get_progress(student_id: int, request: Request, db=Depends(get_db)):
